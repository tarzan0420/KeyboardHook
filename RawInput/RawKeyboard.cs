...
namespace GESAP.Std.LowLayer.Windows.RawInput
{
	public sealed class RawKeyboard
	{
		...
		public string ProcessRawInput(IntPtr hdevice)
		{
			string res_str = "";
            ...
            if (_deviceList.ContainsKey(_rawBuffer.header.hDevice))
            {
				lock (_padLock)
                {
                    ...
                    // Added this to filter devices
                    PreMessageFilter.hDevice = keyPressEvent.DeviceName.ToString();
					res_str = PreMessageFilter.hDevice;
